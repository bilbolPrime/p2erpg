...
contract Shields is Initializable, ERC721EnumerableUpgradeable, AccessControlUpgradeable  {
    ...
    uint8 public constant SHIELD_TYPE = 0;
    ...
    function initialize () public initializer {
        __ERC721_init("P2E RPG Shield", "P2ERPGS");
    ...
    function get(uint256 id) public view returns (uint256) {
        return nftVars[id][SHIELD_TYPE] | 
                nftVars[id][ROLL] << 8;
    }

    function mint(address owner, uint256 shieldType, uint256 roll) public restricted returns (uint256 id) {
        id = totalSupply() + 1;
        nftVars[id][SHIELD_TYPE] = shieldType;
        nftVars[id][ROLL] = roll;
         _safeMint(owner, id);
    }


    function getShieldsOwnedBy(address wallet) public view returns(uint256[] memory chars) {
