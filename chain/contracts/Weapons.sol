...
contract Weapons is Initializable, ERC721EnumerableUpgradeable, AccessControlUpgradeable  {
    ...
    uint8 public constant WEAPON_TYPE = 0;
    ...
    function initialize () public initializer {
        __ERC721_init("P2E RPG Weapon", "P2ERPGW");
    ...
    function get(uint256 id) public view returns (uint256) {
        return nftVars[id][WEAPON_TYPE] | 
                nftVars[id][ROLL] << 8;
    }

    function mint(address owner, uint256 weaponType, uint256 roll) public restricted returns (uint256 id) {
        id = totalSupply() + 1;
        nftVars[id][WEAPON_TYPE] = weaponType;
        nftVars[id][ROLL] = roll;
         _safeMint(owner, id);
    }


    function getWeaponsOwnedBy(address wallet) public view returns(uint256[] memory chars) {
